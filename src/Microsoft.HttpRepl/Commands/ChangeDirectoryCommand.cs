        ...
        protected override Task ExecuteAsync(IShellState shellState, HttpState programState, DefaultCommandInput<ICoreParseResult> commandInput, ICoreParseResult parseResult, CancellationToken cancellationToken)
        {
            ...
            else
            {
                ...
                if (programState.Structure != null)
                {
                    ...
                    // If there's no RequestInfo/Methods AND there's no (sub)DirectoryNames, it must be an
                    // autogenerated directory 
                    if ((s.RequestInfo?.Methods is null || s.RequestInfo.Methods.Count == 0) && (s.DirectoryNames is null || !s.DirectoryNames.Any()))
                    {
                    ...
                    else if (s.RequestInfo?.Methods is null)
                    {
                        thisDirMethod = "[]";
                    }
                    else
